...
Cada arquivo `.md` neste diretório é exposto via `@mcp.resource(uri="okto-pulse://reference/<nome>")` e contém material de referência consultado esporadicamente (errors, knowledge governance, multi-value formats, destructive ops, card types, spec gates).
