...
| `reason=missing_amendment_revision` | Cross-spec regression evidence with no formal `AmendmentHotfixRevision` backing this bug (a label, or manual association does NOT satisfy Path B) | Start Path B step 1 (create/associate the revision — protocol above), then complete lineage + evidence + validator coverage. |
