...
- **Path C — hotfix lane.** Post-closure bug with no executable sprint lane: create or choose a `lane_type="hotfix"` sprint, assign the bug and its regression test card, activate it. Keep the original closed sprint unchanged.
...
| `SpecLockedError` / `"spec is locked"` | Tried to edit a `validated`/`in_progress` spec to add regression coverage for a post-lock bug | Do not unlock the spec. Eligible existing scenario → Path A (protocol above); no eligible scenario or expected behavior changed → Path B. |
