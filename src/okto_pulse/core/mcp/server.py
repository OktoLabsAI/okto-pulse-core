...
def _accept_raw_knowledge_propagation_envelope(value: Any) -> dict[str, Any]:
    """Keep object-shape validation at the host while deferring domain validation."""

    if not isinstance(value, dict):
        # PydanticCustomError has no embedded exception object in ``ctx`` and is
        # therefore safe for the host's generic argument-validation envelope.
        raise PydanticCustomError("dict_type", "Input should be an object")
    return value


KnowledgePropagationEnvelopeInput = Annotated[
    KnowledgePropagationEnvelopeV2,
    PlainValidator(
        _accept_raw_knowledge_propagation_envelope,
        json_schema_input_type=KnowledgePropagationEnvelopeV2,
    ),
]
...
# nor _RESOURCE_REGISTRY). The built-in catalog is composed with edition-owned
# declarations at the composition root, then frozen into an exact-byte snapshot.
# ============================================================================
...
def _build_core_resource_catalog() -> StaticMcpResourceCatalog:
    ...
    from okto_pulse.core.mcp.manifest import render_server_manifest

    specs += (
        McpResourceSpec(
            uri="okto-pulse://server-manifest",
            loader=functools.partial(render_server_manifest, mcp),
            description=(
                "Server version/build and deterministic installed tool count, hash, "
                "and alias inventory."
            ),
            category="server",
            edition="core",
...
def _resource_catalog() -> CompositeMcpResourceCatalog | FrozenMcpResourceCatalog:
...
def _resource_catalog_frozen() -> bool:
    return isinstance(_resource_catalog(), FrozenMcpResourceCatalog)


def resource_catalog_is_frozen() -> bool:
    """Whether the effective resource catalog is an immutable byte snapshot."""

    return _resource_catalog_frozen()


def effective_resource_catalog() -> (
    CompositeMcpResourceCatalog | FrozenMcpResourceCatalog
):
    """The authoritative resolved or frozen effective MCP resource catalog."""

    return _resource_catalog()


def effective_resource_manifest():
    """Return the frozen projection's canonical resource manifest."""

    catalog = _resource_catalog()
    if not isinstance(catalog, FrozenMcpResourceCatalog):
        raise McpResourceCatalogError(
            "registry_unfrozen",
            "resource manifest is unavailable before the projection is frozen",
        )
...
def _build_resource_entry(spec: McpResourceSpec) -> CoreMcpResource:
...
def _stage_resource_entries(
    catalog: McpResourceCatalog,
) -> dict[str, CoreMcpResource]:
    """Build a complete compatibility projection without mutating ``mcp``."""

    return {
        spec.uri: _build_resource_entry(spec)
...
def register_resource_catalog(catalog: McpResourceCatalog) -> None:
    """Validate and atomically publish one additional edition catalog."""

    if _resource_catalog_frozen():
        raise McpResourceCatalogError(
            "registry_frozen",
            "late resource registration or replacement is forbidden",
        )
    current = _resource_catalog()
    if not isinstance(current, CompositeMcpResourceCatalog):
        raise McpResourceCatalogError(
            "registry_invalid_state",
            "resource registration requires a mutable composite catalog",
        )
    candidate = current.with_catalog(catalog)

    # Registration publishes validated declarations only.  Transport-facing
    # compatibility handlers remain on the last complete projection until freeze
    # has snapshotted every loader and generated the canonical manifest.
    register_runtime_value(_RESOURCE_CATALOG_KEY, candidate)


def freeze_resource_catalog() -> FrozenMcpResourceCatalog:
    """Stage and publish one immutable, enabled, exact-byte projection.

    Loader, encoding, and manifest failures occur before either the authoritative
    catalog or its compatibility resource entries change.  Repeated calls return
    the same frozen object.
    """

    current = _resource_catalog()
    if isinstance(current, FrozenMcpResourceCatalog):
        return current
    frozen = freeze_mcp_resource_catalog(current)
    staged_entries = _stage_resource_entries(frozen)
    # Publish compatibility handlers first.  An asynchronous interrupt can then
    # leave, at worst, the registry mutable with already-frozen handlers; it can
    # never expose a frozen authoritative catalog backed by stale handlers.
    mcp._resource_manager._resources = staged_entries
    register_runtime_value(_RESOURCE_CATALOG_KEY, frozen)
    return frozen
...
def reset_resource_catalog_for_tests() -> None:
    ...
    catalog = CompositeMcpResourceCatalog([_build_core_resource_catalog()])
    staged_entries = _stage_resource_entries(catalog)
    register_runtime_value(_RESOURCE_CATALOG_KEY, catalog)
    mcp._resource_manager._resources = staged_entries


# Register the built-in resources with the command catalog and build the initial
# compatibility projection at import.
mcp._resource_manager._resources = _stage_resource_entries(_resource_catalog())
...
async def _safe_spec_update(
    service,
    spec_id: str,
    agent_id: str,
    payload,
    *,
    propagate_spec_locked: bool = False,
):
...
def _kg_direct_permission_denied(
    required_permission: str,
    permission_error: str,
) -> str:
    """Canonical non-enumerating denial for direct KG handlers."""

    message = permission_error
    try:
        detail = json.loads(permission_error)
...
def _mcp_architecture_conflict_error(exc: Any) -> str:
    ...
    return _spec_locked_error(
        SpecLockedError(
            spec_id=str(getattr(exc, "entity_id", "")),
            current_validation_id=getattr(exc, "current_validation_id", None),
        )
    )
...
def _patch_catalog_tool_for_xml_safety(catalog: CoreMcpCatalog) -> None:
    ...
    ``functools.wraps``. The clone hook rebuilds this instance-bound wrapper
    for every runtime catalog instead of copying a closure bound to its source.
    """
    catalog.register_runtime_clone_hook(_patch_catalog_tool_for_xml_safety)
...
def _patch_mcp_tool_for_xml_safety() -> None:
    """Apply XML-safe registration to the active runtime catalog."""

    _patch_catalog_tool_for_xml_safety(mcp.resolve())
...
def _encode_activity_cursor(
    ...
    *,
    ...
    """Backward-compatible facade for the canonical activity cursor v2."""

    from okto_pulse.core.services.analytics_contract import encode_activity_cursor

    return encode_activity_cursor(
        created_at,
        row_id,
        board_id=board_id,
        action=action,
...
def _decode_activity_cursor(
    ...
    *,
    ...
    """Compatibility facade returning only the decoded cursor position."""

    from okto_pulse.core.services.analytics_contract import decode_activity_cursor

    return decode_activity_cursor(
        cursor,
        board_id=board_id,
        action=action,
        card_id=card_id,
...
async def okto_pulse_get_activity_log(
    ...
    raw list. `offset` is honored only on the FIRST page (no `cursor`); once you
    page by `cursor`, its keyset position is authoritative and `offset` is ignored
    to avoid double-skipping.
    """
    ctx = await _get_agent_ctx(board_id)
    if not ctx:
        return _auth_error()
    ...
    # Cursor takes precedence; `offset` is honored only on the first page (below).
    ...
    if cursor:
        ...
        decoded_cursor = decode_activity_cursor(
            cursor,
            board_id=board_id,
            action=action,
            card_id=card_id,
        )
        cursor_pair = decoded_cursor.position
        if not decoded_cursor.valid:
            return json.dumps(
                {
                    "error": "Invalid or incompatible cursor",
                    "error_code": decoded_cursor.error_code or "invalid_cursor",
                    "cursor_version": decoded_cursor.version,
                }
            )

    # `offset` is honored ONLY on the first page (no cursor). A cursor encodes an
    # exclusive keyset position, so pairing it with a non-zero offset would
    # double-skip — the cursor stays authoritative and offset is forced to 0 once
    # paging by cursor.
    effective_offset = offset if not cursor_pair else 0
...
def _knowledge_propagation_request_error(error: Exception) -> str:
    """Project an invalid Knowledge V2 request without leaking Pydantic internals.

    ``KnowledgePropagationEnvelopeV2`` deliberately reuses the domain selection
    contract.  Pydantic wraps a ``KnowledgePropagationContractError`` raised by
    that contract inside ``ValidationError.ctx.error``; serialising the raw
    ``errors()`` list therefore fails because the exception object is not JSON
    serialisable.  Recover the typed domain error when present, otherwise emit a
    bounded, string-only validation envelope.
    """
    ...
    if isinstance(error, KnowledgePropagationContractError):
        return _knowledge_propagation_error(error)

    if isinstance(error, ValidationError):
        raw_issues = error.errors(include_url=False)
        for issue in raw_issues:
            cause = (issue.get("ctx") or {}).get("error")
            if isinstance(cause, KnowledgePropagationContractError):
                return _knowledge_propagation_error(cause)
        ...
        for issue in error.errors(
            ...
            issues.append(
                {
                    "path": location or "knowledge_propagation",
                    "code": str(issue.get("type", "value_error")),
                    "detail": str(issue.get("msg", "invalid value")),
                }
            )
        detail = "; ".join(f"{issue['path']}: {issue['detail']}" for issue in issues)
    else:
        issues = [
            {
    ...
    code = "knowledge_propagation_invalid_request"
    return json.dumps(
        {
            "error": code,
            "code": code,
...
def _spec_locked_error(error: SpecLockedError) -> str:
    """Return the stable MCP error for a content-locked spec mutation."""
    ...
    return json.dumps(
        {
            ...
            "details": {
                "spec_id": error.spec_id,
                "current_validation_id": error.current_validation_id,
    ...
    *,
    ...
    for attempt_index in range(2):
        try:
            ...
            return json.dumps({"error": error.code, **error.to_dict(), **error.facts})
...
async def _mcp_derive_spec_from_refinement_v2(
    *,
    ...
    payload = project_derive_spec_response(result.knowledge_mutation).model_dump(
        mode="json"
    )
    return json.dumps(
        {
            ...
            compact_and_emit(
                {
                    ...
                    **project_cancellation(card),
                    ...
                    "pre_archive_status": getattr(
                        getattr(card, "pre_archive_status", None),
                        "value",
                        getattr(card, "pre_archive_status", None),
                    ),
                    "attachments": [
...
async def okto_pulse_get_task_context(
    ...
    context_scope: Annotated[
        ...
        Field(
            description=(
                "all | gate — use profile=full, context_scope=gate for the "
                "bounded mandatory pre-mutation gate/readiness view"
            )
        ),
    ] = "all",
) -> str:
    """Read a card, linked spec, validations, resources and Q&A. Use `summary`
    for exploration; `profile="full", context_scope="gate"` is the bounded
    mandatory read before status changes. The default `context_scope="all"`
    preserves the historical complete full body. Test cards include their
    scenario completion flow. `gate_readiness` and `reviewer_separation` expose
    active validation blockers without mutation. Profiles:
    okto-pulse://reference/projection-profiles.
    """
    ctx = await _get_agent_ctx(board_id)
    if not ctx:
    ...
    from okto_pulse.core.mcp.context_projection import (
        resolve_task_context_scope,
        unsupported_task_context_scope_error,
    )

    _resolved_context_scope = resolve_task_context_scope(context_scope)
    if _resolved_context_scope is None or (
        _resolved_context_scope == "gate" and _resolved_profile != "full"
    ):
        return json.dumps(
            unsupported_task_context_scope_error(
                context_scope,
    ...
    async with get_unit_of_work_factory_for_mcp()(actor=actor) as uow:
        ...
        result["reviewer_separation"] = {
            "applies_to_task_validation": not (
                card.card_type and card.card_type.value == "test"
            ),
            **separation.to_dict(),
        }

        # R4-IMP3: proactive read-only operational-flow block for test cards (reuses
        # the R4-IMP1 test_card_completion contract). Assemble it BEFORE response
        # projection so profile payload_bytes/truncation describe the response the
        ...
        if card.card_type and card.card_type.value == "test":
            ...
            operational_flow = operational_flow_for_test_card(
                ...
                expected_scenario_ids=[
                    str(value) for value in (card.test_scenario_ids or [])
                ],
                evidence_validator=(
                    (
                        lambda scenario: scenario_has_authenticated_required_evidence(
                            board_id=board_id,
                            spec_id=card.spec_id,
                            scenario=scenario,
                    ...
                    if spec is not None and card.spec_id is not None
                    else None
                ),
            )
        ...
        # active gate and its required tool BEFORE acting. It is part of the assembled
        # payload before projection and is preserved by the budget-aware essential
        # projection. No state-machine change; mutation_allowed=False.
        ...
        result["gate_readiness"] = task_gate_readiness(
...
async def okto_pulse_move_card(
    ...
    conclusion: Annotated[
        str, Field(description="Execution report — required for validation/done moves")
    ] = "",
    completeness: Annotated[
        int, Field(description="0-100; -1 when no execution report is required")
    ] = -1,
    completeness_justification: str = "",
    drift: Annotated[
        int, Field(description="0-100 deviation from spec; -1 when not required")
    ] = -1,
            ...
            return json.dumps(
                {
                    "error": e.code,
                    **e.to_dict(),
                    "blocked_by_dependencies": (e.code == "dependencies_incomplete"),
                }
            )
...
async def okto_pulse_add_card_dependency(
    ...
    # MCP-FU6 strangler: reuse the REST AddCardDependencyUseCase. Missing or
    # foreign endpoints are a non-disclosing not-found; duplicate/cycle/self-ref
    # are conflicts. Tool no longer opens get_db_for_mcp nor builds CardService.
    actor = MCPAdapterContract.actor(ctx, board_id=board_id)
    try:
        async with get_unit_of_work_factory_for_mcp()(actor=actor) as uow:
        ...
        return json.dumps({"error": exc.code, **exc.to_dict()})
    return json.dumps(
        {
            "success": True,
            "card_id": card_id,
...
async def okto_pulse_remove_card_dependency(
    ...
    try:
        ...
        return json.dumps({"error": "Card not found"})
    # Keep the historical non-enumerating envelope: a missing, foreign, or
    # already-removed target must remain indistinguishable at the MCP boundary.
    return json.dumps({"success": bool(removed)})
...
def _invalid_window_error(offset: Any, limit: Any) -> str | None:
    """Structured envelope for an out-of-range pagination window, else ``None``.

    Called at the MCP boundary BEFORE the ``min(limit, 200)`` clamp so a non-int
    ``limit``/``offset`` (e.g. a string → ``TypeError`` at the clamp, or a float
    that the clamp would silently coerce to ``200`` and accept) is rejected with a
    structured error instead of crashing or being wrongly accepted. Enforces
    strict ``int`` type, ``offset >= 0`` and ``limit >= PAGE_LIMIT_MIN``; a valid
    ``int > 200`` passes here and is clamped to ``PAGE_LIMIT_MAX`` afterwards.
    """
    from okto_pulse.core.ports.application_persistence import (
        PAGE_LIMIT_MAX,
        PAGE_LIMIT_MIN,
    ...
    if (
        ...
        # offset > PAGE_OFFSET_MAX would overflow SQLite's signed 64-bit OFFSET.
        return _structured_error(
            "page_request_invalid_window",
            [],
            None,
            f"offset must be an int in 0..{PAGE_OFFSET_MAX}",
        )
    if not isinstance(limit, int) or isinstance(limit, bool) or limit < PAGE_LIMIT_MIN:
        return _structured_error(
            "page_request_invalid_window",
            [],
            None,
            (
                f"limit must be an int >= {PAGE_LIMIT_MIN}; values above "
                f"{PAGE_LIMIT_MAX} are clamped"
            ),
        )
    return None


def _kg_pagination_window(
    limit: Any, offset: Any
) -> tuple[int | None, int | None, str | None]:
    """Validate KG drill-down pagination without conflating type and range."""
    ...
    if not 1 <= limit <= 200 or not 0 <= offset <= PAGE_OFFSET_MAX:
        return (
            None,
            None,
            json.dumps(
                {
                    "error": "invalid_pagination_range",
                    "code": "invalid_pagination_range",
                    "detail": (
                        f"limit must be in 1..200 and offset in 0..{PAGE_OFFSET_MAX}"
                    ),
                    "details": {
                        "limit": limit,
                        "offset": offset,
                        ...
                        "limit_maximum": 200,
                        "offset_minimum": 0,
                        "offset_maximum": PAGE_OFFSET_MAX,
                    },
                    "retryable": False,
                }
            ),
...
async def okto_pulse_list_cards_by_status(
    ...
    allowed_statuses = {"", "open", *(item.value for item in CardStatus)}
    if status not in allowed_statuses:
        return _structured_error(
            "invalid_filter",
            sorted(value for value in allowed_statuses if value),
            None,
            f"status={status!r} is not a valid card status filter",
        )
    allowed_priorities = {"", *(item.value for item in CardPriority)}
    if priority not in allowed_priorities:
        return _structured_error(
            "invalid_filter",
            sorted(value for value in allowed_priorities if value),
            None,
    ...
    # An empty collection is semantic on a list surface; compaction must not
    # erase the collection key and force clients to infer its shape.
    compacted.setdefault("cards", [])
    compacted["include_archived"] = include_archived_value
...
# tr_b25890c4). DEDICATED ROUTING preserves the remaining SPRINT asymmetry:
# SprintQAService takes a raw string and performs no activity-log write. Every
# target now uses its canonical ``{target}.qa.ask`` permission leaf.
# ============================================================================
...
async def _ask_question_impl(
    ...
    *,
    ...
    activity-log action, error message, and the remaining sprint asymmetry) and emits safe
...
async def okto_pulse_delete_question(board_id: str, qa_id: str) -> str:
    ...
    async with get_unit_of_work_factory_for_mcp()(actor=actor) as uow:
        result = await McpDeleteQuestionUseCase().execute(
            McpDeleteQuestionCommand(board_id, qa_id),
            actor=actor,
            uow=uow,
        )
    return json.dumps(result.payload)
...
async def okto_pulse_get_choice_responses(board_id: str, comment_id: str) -> str:
    ...
    async with get_unit_of_work_factory_for_mcp()(actor=actor) as uow:
        result = await McpGetChoiceResponsesUseCase().execute(
            McpGetChoiceResponsesCommand(board_id, comment_id),
            actor=actor,
            uow=uow,
        )
    return json.dumps(result.payload, default=str)
...
def _story_list_payload(story) -> dict:
    """Bounded story summary; full mockup HTML belongs to item/context reads."""
    ...
    return {
        ...
        "status": story.status.value,
        "assignee_id": story.assignee_id,
        "screen_mockups_count": int(getattr(story, "screen_mockups_count", 0) or 0),
        "archived": bool(story.archived),
        "ideation_links_count": int(getattr(story, "ideation_links_count", 0) or 0),
        "created_at": story.created_at.isoformat(),
        "updated_at": story.updated_at.isoformat(),
    }
...
async def okto_pulse_update_ideation(
    ...
    if any((title, description, problem_statement, proposed_approach)):
        required_permissions.append("ideation.entity.edit_fields")
    if assignee_id:
        required_permissions.append("ideation.entity.assign")
    if labels:
        required_permissions.append("ideation.entity.label")
    ...
    # parent board before update + commit; the adapter keeps the IdeationUpdate
    # build and presentation envelope.
    actor = MCPAdapterContract.actor(ctx, board_id=board_id)
    async with get_unit_of_work_factory_for_mcp()(actor=actor) as uow:
        try:
            ideation = (
                await McpUpdateIdeationUseCase().execute(
                    McpUpdateIdeationCommand(ideation_id, board_id, ideation_update),
                    actor=actor,
                    uow=uow,
                )
            ).ideation
...
async def okto_pulse_delete_ideation(board_id: str, ideation_id: str) -> str:
    ...
    async with get_unit_of_work_factory_for_mcp()(actor=actor) as uow:
        _r = await McpDeleteIdeationUseCase().execute(
            McpDeleteIdeationCommand(ideation_id, board_id), actor=actor, uow=uow
        )

    if not _r.deleted:
        return json.dumps({"error": "Ideation not found"})
    return json.dumps({"success": True, "takedown": _r.takedown})
...
async def okto_pulse_evaluate_ideation(
    ...
    # Preserve MCP wire values; the application boundary performs the shared,
    # typed integer/range validation before applying any mutation.
    scope: dict = {}
    if domains != "":
        ...
        async with get_unit_of_work_factory_for_mcp()(actor=actor) as uow:
            snapshot = (
                await McpGetIdeationSnapshotUseCase().execute(
                    McpGetIdeationSnapshotCommand(ideation_id, board_id, version),
                    actor=actor,
                    uow=uow,
                )
            ).snapshot
    except HistoryReadValidationError as exc:
        return json.dumps(exc.to_error_dict())
    if not snapshot:
...
async def okto_pulse_get_ideation_history(
    ...
    async with get_unit_of_work_factory_for_mcp()(actor=actor) as uow:
        try:
            command = McpGetIdeationHistoryCommand(
                ideation_id,
                board_id,
                limit,
                offset,
            )
            result = await McpGetIdeationHistoryUseCase().execute(
                command,
                actor=actor,
                uow=uow,
            )
...
async def okto_pulse_answer_ideation_question(
    ...
    async with get_unit_of_work_factory_for_mcp()(actor=actor) as uow:
        ...
        if _r.selection_error is not None:
            return json.dumps(_r.selection_error.to_error_dict())
        if _r.qa_not_found:
            return json.dumps(
                {
                    "error": "qa_not_found",
            ...
            return json.dumps(
                {
                    ...
                    "resource_propagation": _r.resource_propagation,
                },
                default=str,
            )
    except ArtifactResourceSelectionError as e:
    ...
    async with AsyncExitStack() as stack:
        ...
        return json.dumps(
            project_entity_context(
                result,
                profile=profile,
                tool_name="okto_pulse_get_refinement_context",
            ),
            default=str,
        )
...
async def okto_pulse_update_refinement(
    ...
    if labels:
        required_permissions.append("refinement.entity.label")
    if not required_permissions:
        required_permissions.append("refinement.entity.edit_fields")
    for granular_permission in required_permissions:
        perm_err = _mcp_check_permission(
    ...
    # parent board before update + commit; the adapter keeps the RefinementUpdate
    # build and presentation envelope.
    actor = MCPAdapterContract.actor(ctx, board_id=board_id)
    async with get_unit_of_work_factory_for_mcp()(actor=actor) as uow:
        try:
...
async def okto_pulse_delete_refinement(board_id: str, refinement_id: str) -> str:
    ...
    async with get_unit_of_work_factory_for_mcp()(actor=actor) as uow:
        _r = await McpDeleteRefinementUseCase().execute(
            McpDeleteRefinementCommand(refinement_id, board_id),
            actor=actor,
            uow=uow,
        )
...
async def okto_pulse_get_refinement_history(
    ...
    async with get_unit_of_work_factory_for_mcp()(actor=actor) as uow:
        try:
            command = McpGetRefinementHistoryCommand(
                refinement_id,
                board_id,
                limit,
                offset,
            )
            result = await McpGetRefinementHistoryUseCase().execute(
                command,
                actor=actor,
                uow=uow,
            )
...
async def okto_pulse_get_checklist_binding(board_id: str) -> str:
    ...
    try:
        ...
        return json.dumps(project_ska_contract_error(exc, family="checklist"))
...
async def okto_pulse_append_research_decision(
    ...
    try:
        ...
        return json.dumps(project_ska_contract_error(exc, family="research_decision"))
...
async def okto_pulse_list_research_decisions(
    ...
    try:
        command = ListResearchDecisionsCommand(
            ...
            limit=limit,
            cursor=(
                decode_research_decision_cursor(cursor) if cursor.strip() else None
            ),
            ledger_id=ledger_id.strip() or None,
...
async def okto_pulse_get_spec(board_id: str, spec_id: str) -> str:
    ...
    # MCP-FU6 strangler (spec REUSE): GetSpecUseCase enforces the actor board before
    # exposing the payload (incl. lazy cards and permission-gated IR/OR fields).
    actor = MCPAdapterContract.actor(ctx, board_id=board_id)
    async with get_unit_of_work_factory_for_mcp()(actor=actor) as uow:
        try:
            spec = (
        ...
        payload = {
            ...
            "skip_test_coverage": bool(spec.skip_test_coverage),
            "skip_rules_coverage": bool(getattr(spec, "skip_rules_coverage", False)),
            "skip_trs_coverage": bool(getattr(spec, "skip_trs_coverage", False)),
            ...
            **project_cancellation(spec),
            ...
            "pre_archive_status": getattr(spec, "pre_archive_status", None),
            "cards": [
                {
                    "id": c.id,
                    "title": c.title,
            ...
            checklist_readiness = {
                ...
                "checklist_stale_reasons": (
                    ...
                    if checklist_currentness is None
                        ...
                        reason.value for reason in checklist_currentness.stale_reasons
                    )
                ),
            }
...
async def okto_pulse_update_spec(
    ...
    if assignee_id:
        required_permissions.append("spec.entity.assign")
    if labels:
        required_permissions.append("spec.entity.label")
    if not required_permissions:
        required_permissions.append("spec.entity.edit_fields")
    for granular_permission in required_permissions:
        perm_err = _mcp_check_permission(
...
@mcp.tool()
async def okto_pulse_execute_test_scenario_evidence(
    board_id: str,
    spec_id: str,
    scenario_id: str,
    status: Annotated[
        str,
        Field(description="Target status: automated | passed | failed"),
    ],
    ...
    """Execute a server-owned replay and return authenticated Evidence V2.

    This tool never mutates scenario state. Prefer ``replay`` so an MCP client
    ...
    try:
        ...
        return json.dumps(
            {"error": "not_found", "entity": exc.entity_type, "message": str(exc)}
        )
    except ValueError as exc:
        return json.dumps({"error": "evidence_execution_failed", "message": str(exc)})
    return json.dumps(
        {
            "success": True,
            "persisted": False,
            "scenario_persisted": False,
            "manifest_persisted": bool(replay.strip()),
...
async def okto_pulse_update_test_scenario_status(
    ...
    scenario_id: str,
    status: Annotated[
        str, Field(description="draft | ready | automated | passed | failed")
    ],
    evidence: Annotated[
        str,
        Field(
            description="JSON evidence; passed/failed require an evidence_class payload or a full run-log (NC-9 gate)"
        ),
    ] = "",
) -> str:
...
async def _link_task_to_rule_internal(
    ...
    try:
        async with get_unit_of_work_factory_for_mcp()(actor=actor) as uow:
            result = await LinkCardTraceabilityUseCase().execute(
                LinkCardTraceabilityCommand(spec_id, card_id, [("rule", rule_id)]),
                actor=actor,
                uow=uow,
            )
        ...
        return json.dumps({"error": f"{exc.entity_type.title()} not found"})
    except TraceabilityTargetNotFoundError:
...
async def _link_task_to_fr_internal(
    ...
    actor = MCPAdapterContract.actor(ctx, board_id=board_id)
    try:
        async with get_unit_of_work_factory_for_mcp()(actor=actor) as shared_uow:
            result = await LinkCardTraceabilityUseCase().execute(
                LinkCardTraceabilityCommand(spec_id, card_id, [("fr", fr_id)]),
                actor=actor,
                uow=shared_uow,
            )
            linked_spec = await shared_uow.services.specs.get_spec(spec_id)
            cov = _spec_coverage(linked_spec)
    except EntityNotFoundError as exc:
        return json.dumps({"error": f"{exc.entity_type.title()} not found"})
    except TraceabilityTargetNotFoundError:
        return json.dumps(
...
async def _load_mcp_spec_card_for_link(
    uow: Any,
    actor: Any,
    spec_id: str,
    card_id: str,
) -> tuple[Any | None, Any | None, str | None]:
    """Resolve both link parents inside the authenticated MCP board."""
    from okto_pulse.core.application.use_cases.base import EntityNotFoundError
    from okto_pulse.core.application.use_cases.spec_crud import (
        _require_actor_board_spec,
        _require_spec_board_card,
    )

    try:
        spec = await _require_actor_board_spec(uow.services, spec_id, actor)
        card = await _require_spec_board_card(uow.services, card_id, spec)
    except EntityNotFoundError as exc:
...
async def _link_task_to_contract_internal(
    ...
    async with get_unit_of_work_factory_for_mcp()(actor=actor) as uow:
        spec_service = uow.services.specs
        spec, _, scope_error = await _load_mcp_spec_card_for_link(
            uow, actor, spec_id, card_id
        )
        if scope_error:
            return scope_error
...
async def okto_pulse_archive_tree(
    ...
    if not ctx:
        return _auth_error()
    if entity_type not in {"ideation", "refinement", "spec"}:
        return json.dumps(
            {
                "error": (
                    f"Invalid entity_type: {entity_type}. Must be ideation, "
                    "refinement, or spec."
                )
            }
        )
...
async def okto_pulse_restore_tree(
    ...
    try:
        ...
        return json.dumps({"error": str(exc)})
    return json.dumps({"success": True, "restored_count": result.counts}, default=str)
        ...
        return json.dumps(
            {
                ...
                "detail": f"offset must be an int in 0..{PAGE_OFFSET_MAX}",
            }
        )
...
async def okto_pulse_get_card_knowledge_propagation(
    ...
    try:
        ...
        return _knowledge_propagation_error(error)
    return json.dumps(
        project_technical_read_response(result.read_result).model_dump(mode="json"),
        default=str,
    )
    ...
    try:
        async with get_unit_of_work_factory_for_mcp()(actor=actor) as uow:
            result = await McpCopyQaToCardUseCase().execute(
                McpCopyQaToCardCommand(board_id, spec_id, card_id),
                actor=actor,
                uow=uow,
            )
    except EntityNotFoundError as e:
...
def _parse_expected_spec_version(raw: int | str | None) -> int | None:
    ...
    raise ValueError(
        "expected_spec_version must be a non-negative integer when provided."
    )
...
async def okto_pulse_update_decision(
    ...
    supersedes_clear_requested = (
        bool(supersedes_decision_id)
        and supersedes_decision_id.strip().upper() == "CLEAR"
    )
    if content_update_requested or supersedes_clear_requested:
        requested_permissions.append("spec.structured_entity.decision.update")
    if supersedes_decision_id and not supersedes_clear_requested:
        requested_permissions.append("spec.structured_entity.decision.supersede")
    status_permission = {
        "active": "spec.structured_entity.decision.restore",
        "superseded": "spec.structured_entity.decision.supersede",
        "revoked": "spec.structured_entity.decision.revoke",
    }.get(status)
    if status_permission:
        requested_permissions.append(status_permission)
    if not requested_permissions:
        requested_permissions.append("spec.structured_entity.decision.update")
    for granular_permission in dict.fromkeys(requested_permissions):
        perm_err = _mcp_check_permission(
            ctx.permissions,
            granular_permission,
            Permissions.SPECS_UPDATE,
...
async def _remove_spec_entity_impl(
    ...
    *,
    ...
    granular_permission = {
        "business_rule": "spec.rules.delete",
        "api_contract": "spec.contracts.delete",
        "decision": "spec.structured_entity.decision.revoke",
    }[target_type]
    perm_err = _mcp_check_permission(
        ctx.permissions,
        granular_permission,
...
ScreenMockupEntityType = Literal["spec", "ideation", "refinement", "card", "story"]
SCREEN_MOCKUP_ENTITY_TYPES: tuple[ScreenMockupEntityType, ...] = (
    "spec",
    "ideation",
    "refinement",
    "card",
...
async def okto_pulse_add_screen_mockup(
    ...
    entity_id: str,
    title: str,
    entity_type: ScreenMockupEntityType = "spec",
    description: str = "",
...
async def okto_pulse_list_screen_mockups(
    ...
    """
    List screen mockups with summary projections by default.

    Set include_content=true to include each screen's full HTML content."""
    ctx = await _get_agent_ctx(board_id)
    if not ctx:
        return _auth_error()
...
async def okto_pulse_submit_spec_evaluation(
    ...
    from okto_pulse.core.application.use_cases import GetSpecCommand, GetSpecUseCase
    from okto_pulse.core.application.use_cases.base import EntityNotFoundError
    from okto_pulse.core.inbound.mcp_adapter import MCPAdapterContract
    ...
    async with get_unit_of_work_factory_for_mcp()(actor=actor) as uow:
        ...
        try:
            ...
            return json.dumps({"error": "Spec not found"})
        status_before = spec_before.status.value
        try:
...
async def okto_pulse_add_spec_knowledge(
    ...
    try:
        async with get_unit_of_work_factory_for_mcp()(actor=actor) as uow:
            kb = (
                await CreateSpecKnowledgeUseCase().execute(
                    CreateSpecKnowledgeCommand(
                        spec_id,
                        SpecKnowledgeCreate(
                            title=title,
...
async def okto_pulse_update_sprint(
    ...
    origin_sprint_id: str = "",
    origin_bug_id: str = "",
    skip_test_coverage: OptionalBoolInput = None,
    skip_rules_coverage: OptionalBoolInput = None,
    skip_qualitative_validation: OptionalBoolInput = None,
    validation_threshold: int = 0,
    start_date: str = "",
    end_date: str = "",
    ...
    if expected_outcome:
        kwargs["expected_outcome"] = expected_outcome
    if test_scenario_ids:
        try:
    ...
    if skip_test_coverage is not None:
        kwargs["skip_test_coverage"] = _flag_enabled(skip_test_coverage)
    if skip_rules_coverage is not None:
        kwargs["skip_rules_coverage"] = _flag_enabled(skip_rules_coverage)
    if skip_qualitative_validation is not None:
        kwargs["skip_qualitative_validation"] = _flag_enabled(
            skip_qualitative_validation
        )
    if validation_threshold:
        kwargs["validation_threshold"] = validation_threshold
    if start_date:
            ...
            return json.dumps(
                {
                    ...
                    "sprint": {
                        ...
                        "cancelled_at": isoformat_utc(sprint.cancelled_at),
                    },
                }
            )
    except EntityNotFoundError:
...
async def okto_pulse_get_sprint_context(
    ...
    **Always call this before evaluating, moving, or reviewing a sprint.**"""
    ...
    _inc_spec = _flag_enabled(include_spec)
    ...
    return json.dumps(
        project_entity_context(
            _r.result,
            profile=profile,
            tool_name="okto_pulse_get_sprint_context",
        ),
        default=str,
    )
...
async def okto_pulse_answer_sprint_question(
    ...
    # MCP-FU6 strangler (sprint answer Q&A, VARIANT): NO activity log (unique among
    # the Q&A-answer family), a plain answer string, and an UNCONDITIONAL
    ...
    async with get_unit_of_work_factory_for_mcp()(actor=actor) as uow:
        _r = await McpAnswerSprintQuestionUseCase().execute(
            McpAnswerSprintQuestionCommand(board_id, sprint_id, qa_id, answer),
            actor=actor,
            uow=uow,
        )
        if _r.self_answer_error is not None:
...
async def okto_pulse_suggest_sprints(
    ...
    # MCP-FU6 strangler (sprint suggest, REUSE:SuggestSprintsUseCase — read). Missing or
    # cross-board specs map to the same not-found envelope; readiness ValueError keeps
    # its established {error: str} shape.
    actor = MCPAdapterContract.actor(ctx, board_id=board_id)
    try:
        async with get_unit_of_work_factory_for_mcp()(actor=actor) as uow:
...
def _template_spec_checklist_mode(template: Any) -> str:
    ...
    return (
        value
        if value in {"off", "advisory", "blocking"}
...
async def okto_pulse_list_default_board_config_versions(
    ...
    scope: str = "global",
    offset: PageWindowInput = 0,
    limit: PageWindowInput = 20,
) -> str:
    """List a bounded page of default board-config versions and the active id.
    Default limit is 20; integer values above 200 are clamped."""
    ctx = await _get_agent_ctx(board_id)
    if not ctx:
        return _auth_error()
    perm_err = check_permission(ctx.permissions, Permissions.BOARD_READ)
...
@mcp.tool()
async def okto_pulse_list_design_systems(
    board_id: str,
    scope: str = "global",
    limit: int = 50,
    cursor: str = "",
    ...
    try:
        async with get_unit_of_work_factory_for_mcp()(actor=actor) as uow:
            result = await McpDeleteDesignSystemUseCase().execute(
                McpDeleteDesignSystemCommand(board_id, design_system_id),
                actor=actor,
                uow=uow,
            )
        return json.dumps(result.data, default=str)
...
from okto_pulse.core.mcp.kg_query_tools import (  # noqa: E402
...
from okto_pulse.core.mcp.kg_power_tools import (  # noqa: E402
...
from okto_pulse.core.mcp.kg_export_tools import (  # noqa: E402
...
async def okto_pulse_kg_health_readiness(
    ...
    try:
        ...
        return json.dumps(unsupported_projection_error(profile))
    except BoardNotFoundError as exc:
        return json.dumps({"error": str(exc)})
    return json.dumps(result.data, default=str)
...
@mcp.tool()
async def okto_pulse_kg_digest_layer_reconcile(
    board_id: str,
    reason: Annotated[
        str,
        Field(
            description=(
                "Required 3-128 character audit code (lowercase letters, digits, "
                "dot, colon, underscore or hyphen)."
            )
        ),
    ],
) -> McpToolOutcome:
    """Enqueue board-scoped DecisionDigest layer reconciliation.

    Administrative WRITE. Use after the mismatch drill-down reports drift while
    the operational queue is idle. The command emits the established durable
    zero-reference parity event; it does not rebuild the KG or mutate the
    read-only mismatch tool. The worker keyset-inventories publishable board
    sources with duplicate detection, guards stale prune against derived edges,
    repairs identities/Board links, and backfills missing digests. It ACKs each
    ...
    try:
        async with get_unit_of_work_factory_for_mcp()(actor=actor) as uow:
            result = await ReconcileDigestLayerUseCase().execute(
                ReconcileDigestLayerCommand(board_id, reason=reason),
                actor=actor,
                uow=uow,
            )
    except CommandValidationError as exc:
...
async def okto_pulse_kg_dead_letter_list(
    ...
    try:
        # Reject an offset above int64 before it reaches the DLQ reader, whose
        # ``limit + offset`` would otherwise overflow SQLite's LIMIT binding.
        offset = bounded_page_offset(offset)
    except (TypeError, ValueError):
        return json.dumps(
            {
...
async def okto_pulse_kg_tick_run_now(
    ...
    """
    # Authorize before resolving the lease provider or opening a UoW. A global
    # tick is an administrative cross-board write and therefore must use the
    # global effective permission context; a raw authenticated principal is not
    # sufficient.
    if board_id:
        ctx = await _get_agent_ctx(board_id)
        if ctx is None:
...
# ============================================================================
# GLOBAL DISCOVERY RECOVERY
#
# Separate global scope from board rebuild.  This is the governed escape hatch
# for an unreadable discovery.lbug/WAL while authoritative board graphs remain
# healthy. Source inventory/preparation uses the cancellation-safe blocking
# bridge; physical candidate construction/cutover belongs to the owned edition
# worker and never to the MCP request lifetime.
# ============================================================================

_global_recovery_logger = logging.getLogger("okto_pulse.mcp.global_discovery_recovery")
...
def _global_recovery_service():
    ...
    return GlobalDiscoveryRecoveryService(
        recovery=registry.require_global_discovery_recovery(),
        artifact_store=registry.require_rebuild_audit_artifact_store(),
    )


def _global_recovery_control_plane():
    from okto_pulse.core.kg.global_discovery_recovery_control import (
        resolve_recovery_control_plane,
    )

    return resolve_recovery_control_plane()


def _global_recovery_control_error(exc: Exception) -> str:
    """Serialize the closed control-plane error vocabulary without leakage."""
    ...
    if isinstance(exc, RecoveryCheckpointConflict):
        return json.dumps(
            {
                "error": exc.code,
                "run_id": exc.run_id,
                "expected_epoch": exc.expected_epoch,
                "actual_epoch": exc.actual_epoch,
                "expected_progress_seq": exc.expected_progress_seq,
                "actual_progress_seq": exc.actual_progress_seq,
            }
        )
    if isinstance(exc, RecoveryRunNotFound):
        return json.dumps({"error": exc.code, "run_id": exc.run_id})
    if isinstance(exc, RecoveryBindingConflict):
        return json.dumps({"error": exc.code, "run_id": exc.run_id})
    if isinstance(exc, RecoveryResumeRejected):
        return json.dumps({"error": exc.code, "run_id": exc.run_id, "epoch": exc.epoch})
    if isinstance(exc, RecoveryDispatchClaimConflict):
        return json.dumps({"error": exc.code, "run_id": exc.run_id, "epoch": exc.epoch})
    if isinstance(exc, RecoveryInProgress):
        return json.dumps({"error": exc.code})
    if isinstance(exc, RecoveryPhaseIncoherent):
        return json.dumps({"error": exc.code, "state": exc.state, "phase": exc.phase})
    if isinstance(exc, RecoveryAuditReasonInvalid):
        return json.dumps({"error": exc.code})
    if isinstance(exc, RecoveryProgressInvariantViolation):
        return json.dumps({"error": exc.code})
    if isinstance(exc, RecoveryControlPlaneUnavailable):
        return json.dumps({"error": exc.code})
    if isinstance(exc, ValueError):
        return json.dumps({"error": "invalid_recovery_control_request"})
    raise exc


def _global_recovery_status(*, control, run_id: str):
    """Return one global run after the caller passed global-admin auth."""

    return control.status(run_id)


async def _global_recovery_delivery(
    ctx,
    board_pairs: list[tuple[str, str, str]],
    *,
    run_id: str,
) -> dict[str, object]:
    ...
    actor = MCPAdapterContract.actor(ctx)
    async with get_unit_of_work_factory_for_mcp()(actor=actor) as uow:
        result = await uow.services.kg.recover_global_discovery_delivery(
            run_id=run_id,
            board_ids=[board_id for board_id, _name, _summary in board_pairs],
            dead_letter_limit=5000,
        )
        await uow.commit()
    return {**result, "worker_signaled": signal_runtime_worker("outbox_worker")}


async def _global_recovery_finish_delivery(
    ctx,
    board_pairs: list[tuple[str, str, str]],
    *,
    run_id: str,
    ...
    delivery = await _global_recovery_delivery(
        ctx,
        board_pairs,
        run_id=run_id,
    )
    return await run_blocking_graph_io(
        lambda: _global_recovery_service().finalize_delivery(
            run_id=run_id,
            actor_id=ctx.agent_id,
        ...
        task_name="mcp.global_discovery_recovery.finalize_delivery",
    )


async def _global_recovery_drain_async(operation, *, task_name: str):
    """Drain an owned async UoW/finalization task across repeated cancellation."""
    ...
    if error is not None:
        if cancelled is not None:
            _global_recovery_logger.exception(
                "global recovery drained delivery failed after cancellation",
                exc_info=error,
            )
            raise cancelled
        raise error
...
def _global_recovery_should_finish_delivery_after_cancel(
    status: dict[str, object] | None,
) -> bool:
    """Never reopen DLQ when cancelled native work failed or rolled back."""

    return bool(status is not None and status.get("state") == "delivery_pending")


def _global_recovery_delivery_board_pairs(
    status: dict[str, object] | None,
    fallback: list[tuple[str, str, str]],
) -> list[tuple[str, str, str]]:
    raw_ids = status.get("board_ids") if status is not None else None
    if not isinstance(raw_ids, list):
        return fallback
    board_ids = sorted(
        {value.strip() for value in raw_ids if isinstance(value, str) and value.strip()}
    )
    return [(board_id, board_id, "") for board_id in board_ids] or fallback


_global_outbox_dead_letter_logger = logging.getLogger(
    "okto_pulse.mcp.global_outbox_dead_letter"
)


def _global_outbox_dead_letter_error_response(
    operation: str,
    exc: Exception,
    *,
    mutation: bool,
) -> str:
    """Serialize the closed DLQ error vocabulary without backend leakage."""

    from okto_pulse.core.application.global_outbox_dead_letter import (
        GlobalOutboxDeadLetterError,
    )

    if isinstance(exc, GlobalOutboxDeadLetterError):
        return json.dumps(exc.to_dict())

    error_text = str(exc).lower()
    if type(exc).__name__ == "OperationalError" and any(
        ...
        _global_outbox_dead_letter_logger.warning(
            "global-outbox DLQ operation=%s rejected because the relational "
            "store is busy",
            operation,
        )
        return json.dumps({"error": "global_outbox_busy", "mutated": False})

    _global_outbox_dead_letter_logger.error(
        "global-outbox DLQ operation=%s failed with %s",
        operation,
        type(exc).__name__,
        exc_info=(type(exc), exc, exc.__traceback__),
    )
    payload: dict[str, object] = {
        "error": f"global_outbox_dead_letter_{operation}_failed"
    }
    if mutation:
...
async def okto_pulse_kg_global_outbox_dead_letter_list(
    limit: Annotated[
        int,
        Field(ge=1, le=100, description="Page size from 1 through 100"),
    ] = 50,
    ...
    classification: Annotated[
        ...
        Field(
            description=(
                "Optional global_open_failure, board_source_failure, or "
                "unclassified_failure filter"
            )
        ),
    ] = None,
) -> str:
...
@mcp.tool()
async def okto_pulse_kg_global_outbox_dead_letter_reprocess(
    ...
    try:
        async with get_unit_of_work_factory_for_mcp()(actor=actor) as uow:
            result = await uow.services.kg.reprocess_global_outbox_dead_letters(
                dead_letter_ids=dead_letter_ids,
                reason=reason,
            )
            await uow.commit()
    except Exception as exc:
        return _global_outbox_dead_letter_error_response(
            "reprocess", exc, mutation=True
        )
    ...
    if process_now:
        ...
        try:
            worker_signaled = bool(signal_runtime_worker("outbox_worker"))
        except Exception as exc:
            _global_outbox_dead_letter_logger.error(
                "global-outbox DLQ replay committed but worker signal failed with %s",
                type(exc).__name__,
            )
    return json.dumps(
        {**result, "worker_signaled": worker_signaled},
        default=str,
    )


@mcp.tool()
async def okto_pulse_kg_global_outbox_dead_letter_verify(
    dead_letter_ids: list[str],
) -> str:
    """KG docs."""
    ...
    try:
        async with get_unit_of_work_factory_for_mcp()(actor=actor) as uow:
            result = await uow.services.kg.verify_global_outbox_dead_letters(
                dead_letter_ids=dead_letter_ids,
            )
    except Exception as exc:
        return _global_outbox_dead_letter_error_response("verify", exc, mutation=False)
    return json.dumps(result, default=str)
...
async def okto_pulse_kg_global_discovery_recovery_preflight() -> str:
    ...
    service = _global_recovery_service()
    command = service.new_preparation_command(actor_id=ctx.agent_id)
    try:
        status = await asyncio.wait_for(
            asyncio.to_thread(
                _global_recovery_control_plane().prepare,
                command,
            ),
            timeout=_GLOBAL_RECOVERY_PREFLIGHT_TIMEOUT_SECONDS,
        )
    except TimeoutError:
        return json.dumps(
            {
        ...
        return json.dumps({"error": exc.code, "reason": exc.reason})
    except Exception as exc:
        try:
            return _global_recovery_control_error(exc)
        except Exception:
            _global_recovery_logger.exception("global recovery preflight failed")
            return json.dumps(
                {
                    "error": "global_discovery_recovery_preflight_failed",
                    "detail": f"unexpected_internal_error:{type(exc).__name__}",
                }
            )
    ...
    action_required = (
        ...
        else "call_okto_pulse_kg_global_discovery_recovery_status"
    )
    return json.dumps(
        {
            **payload,
...
@mcp.tool()
async def okto_pulse_kg_global_discovery_recovery_confirm(
    run_id: str,
    manifest_ref: str,
    preflight_hash: str,
) -> str:
    ...
    try:
        ...
        _global_recovery_logger.exception("global recovery confirm failed")
        return json.dumps(
            {
                "error": "global_discovery_recovery_confirm_failed",
                "detail": f"unexpected_internal_error:{type(exc).__name__}",
            }
        )
    return json.dumps(result, default=str)
...
async def okto_pulse_kg_global_discovery_recovery_status(run_id: str) -> str:
    ...
    try:
        status = await run_blocking_graph_io(
            lambda: _global_recovery_status(
                control=_global_recovery_control_plane(),
                run_id=run_id,
            ),
            task_name="mcp.global_discovery_recovery.status",
        )
    except Exception as exc:
        try:
            return _global_recovery_control_error(exc)
        except Exception:
            _global_recovery_logger.exception("global recovery status failed")
            return json.dumps(
                {
                    "error": "global_discovery_recovery_status_failed",
                    "detail": f"unexpected_internal_error:{type(exc).__name__}",
                }
            )
...
@mcp.tool()
async def okto_pulse_kg_global_discovery_recovery_cancel(
    run_id: str,
    expected_epoch: int,
    reason: Annotated[
        str | None,
        Field(
            max_length=512,
            description="Optional operator audit reason (at most 512 characters)",
        ),
    ] = None,
) -> str:
    """See KG docs."""
    ...
    def request_cancel():
        control = _global_recovery_control_plane()
        _global_recovery_status(control=control, run_id=run_id)
        return control.cancel(
            run_id=run_id,
            expected_epoch=expected_epoch,
            requested_at=datetime.now(timezone.utc),
            requested_by_actor_id=ctx.agent_id,
            reason=reason,
        )
    ...
    try:
        status = await run_blocking_graph_io(
            request_cancel,
            task_name="mcp.global_discovery_recovery.cancel",
        )
    except Exception as exc:
        try:
            return _global_recovery_control_error(exc)
        except Exception:
            _global_recovery_logger.exception("global recovery cancel failed")
            return json.dumps(
                {
                    "error": "global_discovery_recovery_cancel_failed",
                    "detail": f"unexpected_internal_error:{type(exc).__name__}",
                }
            )
    return json.dumps(status.to_dict())
...
@mcp.tool()
async def okto_pulse_kg_global_discovery_recovery_resume(
    run_id: str,
    expected_epoch: int,
    reason: Annotated[
        str | None,
    ...
    try:
        status = await run_blocking_graph_io(
            request_resume,
            task_name="mcp.global_discovery_recovery.resume",
        )
    except Exception as exc:
        try:
            return _global_recovery_control_error(exc)
        except Exception:
            _global_recovery_logger.exception("global recovery resume failed")
            return json.dumps(
                {
                    "error": "global_discovery_recovery_resume_failed",
                    "detail": f"unexpected_internal_error:{type(exc).__name__}",
                }
            )
    return json.dumps(status.to_dict())
...
@mcp.tool()
async def okto_pulse_kg_global_discovery_recovery_run(
    confirmation_id: str,
    manifest_ref: str,
    preflight_hash: str,
    reason: Annotated[
        str,
        Field(min_length=1, max_length=512, description="Operator audit reason"),
    ],
) -> str:
    """See KG docs."""
    ctx, error = await _global_recovery_authorize()
    ...
    service = _global_recovery_service()
    try:
        command = await run_blocking_graph_io(
            lambda: service.prepare_durable_start(
                actor_id=ctx.agent_id,
        ...
        def dispatch_prepared_run():
            control = _global_recovery_control_plane()
            before = control.status(command.binding.run_id)
            status = control.start(command)
            return status, before.phase != "prepared"

        status, idempotent_replay = await run_blocking_graph_io(
            dispatch_prepared_run,
            task_name="mcp.global_discovery_recovery.start.dispatch",
        )
    except GlobalDiscoveryRecoveryError as exc:
        return json.dumps({"error": exc.code, "reason": exc.reason})
    except Exception as exc:
        try:
            ...
            _global_recovery_logger.exception("global recovery start failed")
            return json.dumps(
                {
                    "error": "global_discovery_recovery_run_failed",
                    "detail": f"unexpected_internal_error:{type(exc).__name__}",
                }
            )
    return json.dumps(
        {
            **status.to_dict(),
            ...
            "action_required": "call_okto_pulse_kg_global_discovery_recovery_status",
        }
    )
...
async def okto_pulse_kg_rebuild_preflight(
    ...
    if _raw_health.get("graph_state") == "healthy" and (
        bool(_raw_health.get("discovery_recovery_required"))
        or _raw_health.get("discovery_state") in {"recovery_needed", "quarantined"}
    ):
        return json.dumps(
            {
                "error": "board_rebuild_wrong_recovery_scope",
                "outcome": "redirected",
                "action_required": (
                    "call_okto_pulse_kg_global_discovery_recovery_preflight"
                ),
                "reason": (
                    "the board graph is healthy; the failed store is the "
                    "global discovery cache"
                ),
                "graph_state": _raw_health.get("graph_state"),
                "discovery_state": _raw_health.get("discovery_state"),
            }
        )
...
async def _run_rebuild_service_cooperatively(
    ...
    **run_kwargs: Any,
    ...
    """Run the blocking rebuild while preserving cancellation cleanup."""
    ...
    try:
        # Shield keeps the worker future observable after the MCP request is
        # cancelled.  The cooperative probe then drives compensation and lock
        # release inside the rebuild thread instead of abandoning it.
        ...
        def _consume_background_result(task: asyncio.Task[Any]) -> None:
            try:
                background_result = task.result()
            except asyncio.CancelledError:
                return
            except Exception as exc:
                _rebuild_logger.error(
                    "kg.rebuild.run.cleanup_failed board=%s err=%s",
                    board_id,
                    exc,
                )
                return
            _rebuild_logger.info(
                "kg.rebuild.run.cancelled_cleanup_done board=%s outcome=%s run_id=%s",
                board_id,
                background_result.outcome,
                background_result.run_id,
            )

        run_task.add_done_callback(_consume_background_result)
        _rebuild_logger.warning(
            "kg.rebuild.run.cancel_requested board=%s; cleanup continues",
            board_id,
        )
        raise
...
async def okto_pulse_kg_quarantine_restore(
    ...
    # The quarantine id does not reveal its owning board. Resolve the minimum
    # plan only after global-admin authorization, then re-authorize against the
    # destination board's effective overrides before returning plan metadata or
    # mutating any file.
...
async def okto_pulse_list_by_board(
    ...
    status_type = status_types.get(entity_type)
    if status_value not in (None, "") and status_type is not None:
        try:
            status_type(str(status_value))
        except ValueError:
            allowed = [item.value for item in status_type]
            return _structured_error(
                "invalid_filter",
                allowed,
                None,
                (
                    f"status={status_value!r} is not valid for "
                    ...
                    "items": [
                        {
                            ...
                            "labels": i.labels,
                            "archived": bool(getattr(i, "archived", False)),
                            "pre_archive_status": getattr(
                                getattr(i, "pre_archive_status", None),
                                "value",
                                getattr(i, "pre_archive_status", None),
                            ),
                    ...
                    "items": [
                        {
                            ...
                            "pre_archive_status": getattr(
                                getattr(r, "pre_archive_status", None),
                                "value",
                                getattr(r, "pre_archive_status", None),
                            ),
            ...
            return json.dumps(
                {
                    ...
                    "limit": limit,
                    "include_archived": bool(filters.get("include_archived", False)),
                    "items": [_story_list_payload(s) for s in paginated],
                },
                default=str,
            )
...
async def okto_pulse_list_qa(
    ...
    # Value-check the answer-state filter: keys are validated above; the VALUE is
    # fail-closed here so an unknown status returns a structured envelope instead
    # of being silently ignored. The use case enforces the same vocabulary.
    from okto_pulse.core.application.use_cases.mcp_mockups_copy_lists import (
        QA_STATUS_VALUES,
    )

    status_value = (filters or {}).get("status")
    if (
        status_value is not None
        and str(status_value).strip().lower() not in QA_STATUS_VALUES
    ):
        return _structured_error(
            "invalid_filter",
            sorted(QA_STATUS_VALUES),
            None,
            f"status='{status_value}' is not a valid Q&A answer-state filter",
        )
    ...
    try:
        async with get_unit_of_work_factory_for_mcp()(actor=actor) as uow:
            result = await McpListQaUseCase().execute(
                McpListQaCommand(board_id, entity_type, entity_id, filters or {}),
                actor=actor,
                uow=uow,
            )
    except EntityNotFoundError as exc:
    ...
    try:
        async with get_unit_of_work_factory_for_mcp()(actor=actor) as uow:
            result = await McpListSnapshotsUseCase().execute(
                McpListSnapshotsCommand(board_id, entity_type, entity_id),
                actor=actor,
                uow=uow,
            )
    except EntityNotFoundError as exc:
...
def build_mcp_asgi_app(trace_sink: McpTraceSink | None = None):
    ...
    resource_catalog = effective_resource_catalog()
    if not isinstance(resource_catalog, FrozenMcpResourceCatalog):
        raise McpResourceCatalogError(
            "registry_unfrozen",
            "MCP host construction requires a frozen resource projection",
        )
    return get_mcp_host_provider().build_asgi_app(
        mcp,
        resource_catalog=resource_catalog,
        projection_identity=resource_catalog.identity,
...
def mount_mcp(
    ...
    *,
    ...
    if not isinstance(resource_catalog, FrozenMcpResourceCatalog):
        raise McpResourceCatalogError(
            "registry_unfrozen",
            "MCP host mounting requires a frozen resource projection",
        )
