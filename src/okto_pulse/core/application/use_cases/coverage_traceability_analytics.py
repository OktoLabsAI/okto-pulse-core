"""Permission-scoped canonical coverage and traceability projection."""
...
class CoverageTraceabilityAnalyticsCommand:
    board_id: str
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.board_id, str) or not self.board_id.strip():
            raise ValueError("coverage_traceability_board_id_required")
        if not isinstance(self.window, AnalyticsUtcWindow):
            raise ValueError("coverage_traceability_window_required")
        object.__setattr__(
            self,
            "as_of",
...
@dataclass(frozen=True, slots=True)
class CoverageTraceabilityAnalyticsResult:
    data: dict[str, object]


class CoverageTraceabilityAnalyticsUseCase:
    async def execute(
        self,
        command: CoverageTraceabilityAnalyticsCommand,
        *,
        actor: ActorContext,
        uow: PulseUnitOfWork,
        ...
        projection = await uow.services.analytics.canonical_coverage(
            query=query,
            as_of=command.as_of,
        )
        return CoverageTraceabilityAnalyticsResult(projection.canonical_dict())
