...
def _relational_projection_candidate_id(source_artifact_ref: str) -> str:
    return relational_projection_candidate_id(source_artifact_ref)
...
def _relational_projection_edge_id(
    ...
    return relational_projection_edge_id(
        edge_type,
        from_candidate_id,
        to_candidate_id,
    )
...
def _project_research_decisions(
    *,
    ...
    for item in sorted(
        ...
        decision_ref = f"refinement:{refinement_id}:rdl:{ledger_id}:decision"
        ...
        result.edges.extend(
            (
                EmittedEdge(
                    ...
                    confidence=1.0,
                    rule_id=relational_projection_belongs_to_rule("Decision"),
                ),
        ...
        for alternative_hash, alternative in sorted(normalized_alternatives.items()):
            alternative_ref = (
                f"refinement:{refinement_id}:rdl:{ledger_id}:"
                f"alternative:{alternative_hash}"
            )
            alternative_cid = _relational_projection_candidate_id(alternative_ref)
            result.nodes.append(
                EmittedNode(
            ...
            result.edges.extend(
                (
                    EmittedEdge(
                        ...
                        to_candidate_id=refinement_candidate_id,
                        confidence=1.0,
                        rule_id=relational_projection_belongs_to_rule("Alternative"),
                    ),
                    EmittedEdge(
                        candidate_id=_relational_projection_edge_id(
                            "relates_to",
                        ...
                        confidence=1.0,
                        rule_id=relational_projection_alternative_relation_rule(),
                    ),
                )
            )
    ...
    *,
    ...
    for arch_index, raw_design in enumerate(architecture_designs or []):
        ...
        for interface_index, raw_interface in enumerate(
            ...
            name = (
                raw_interface.get("name")
                or f"Architecture interface {interface_index + 1}"
            )
...
class DeterministicWorker:
    ...
    def process_spec(self, spec: dict[str, Any]) -> WorkerResult:
        ...
        quality_assessments = _projection_records(spec.get("quality_assessments"))
        ...
        if "spec_dependencies" in spec:
            raw_parts.append(_projection_raw("spec_dependencies", spec_dependencies))
        ...
        if "spec_dependencies" in spec:
            for dependency in spec_dependencies:
                ...
                dependent_spec_id = str(dependency.get("dependent_spec_id") or "")
                prerequisite_spec_id = str(dependency.get("prerequisite_spec_id") or "")
                if (
                    not dependency_id
                ...
                prerequisite_cid = f"kgref:Entity:spec:{prerequisite_spec_id}"
                edge_cid = f"{prefix}_precedes_{dependency_id}"
                rule_id = f"precedes/spec_dependency/{dependency_id}@{WORKER_VERSION}"
                result.edges.append(
                    EmittedEdge(
        ...
        for i, req in enumerate(spec.get("functional_requirements") or []):
            text = (
                ...
                else (req.get("text") or req.get("description") or json.dumps(req))
            )
        ...
        for i, ir in enumerate(spec.get("integration_requirements") or []):
            ...
            content_parts = [
                ...
                f"type={ir.get('integration_type')}"
                if ir.get("integration_type")
                else "",
                ...
                f"contract_ref={ir.get('contract_ref')}"
                if ir.get("contract_ref")
                else "",
            ]
        ...
        logger.info(
            ...
            len(result.nodes),
            len(result.edges),
            len(result.missing_link_candidates),
            result.deterministic_edge_ratio(),
            extra={
    ...
    def process_ideation(self, ideation: dict[str, Any]) -> WorkerResult:
        ...
        result = WorkerResult()
        quality_assessments = _projection_records(ideation.get("quality_assessments"))
        raw_parts = [
            ...
            json.dumps(
                ideation.get("scope_assessment") or {},
                ensure_ascii=False,
                sort_keys=True,
            ),
            json.dumps(
                ideation.get("labels") or [], ensure_ascii=False, sort_keys=True
            ),
        ]
        ...
        if isinstance(story_ids, str):
            story_ids = [story_ids]
        for idx, story_id in enumerate(
            str(s) for s in story_ids if s not in (None, "")
        ):
            result.edges.append(
                EmittedEdge(
    ...
    def process_refinement(self, refinement: dict[str, Any]) -> WorkerResult:
        ...
        result = WorkerResult()
        quality_assessments = _projection_records(refinement.get("quality_assessments"))
        research_decisions = _projection_records(refinement.get("research_decisions"))
        raw_parts = [
            ...
            json.dumps(
                refinement.get("decisions") or [], ensure_ascii=False, sort_keys=True
            ),
            json.dumps(
                refinement.get("labels") or [], ensure_ascii=False, sort_keys=True
            ),
        ]
    ...
    def process_card(self, card: dict[str, Any]) -> WorkerResult:
        ...
        logger.info(
            ...
            len(result.missing_link_candidates),
            extra={
                "event": "deterministic_worker.card_processed",
                "card_id": cid,
                "card_type": card_type,
                "content_hash": result.content_hash,
                "worker_version": WORKER_VERSION,
    ...
    def process_amendment(self, amendment: dict[str, Any]) -> WorkerResult:
        ...
        if regression_scenario_ids:
            content_lines.append(
                ...
                + json.dumps(
                    regression_scenario_ids, ensure_ascii=False, sort_keys=True
                )
            )
        ...
        logger.info(
            ...
            len(result.edges),
            extra={
                "event": "deterministic_worker.amendment_processed",
                "amendment_id": aid,
                "content_hash": result.content_hash,
                "graph_layer": graph_layer,
                "worker_version": WORKER_VERSION,
    ...
    def process_code_investigation_receipt(
        ...
        trust_level = _required_traceability_choice(
            receipt,
            "trust_level",
            allowed=frozenset({"single_attestation", "corroborated", "conflicted"}),
        )
        ...
        if superseded_id is not None:
            result.edges.append(
                EmittedEdge(
                    ...
                    confidence=1.0,
                    rule_id=(f"supersedes/code_traceability_evidence@{WORKER_VERSION}"),
                )
            )
        graph_layer, maturity_status = _layer_attrs_for_artifact(
            "code_evidence",
    ...
    def process_implementation_target(
        ...
        safe_payload = {
            ...
            "resolved_qualified_symbol": target.get("resolved_qualified_symbol"),
        ...
        if (resolved_line_start is None) != (resolved_line_end is None) or (
            ...
            and (resolved_line_start < 1 or resolved_line_end < resolved_line_start)
        ):
