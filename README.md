...
- **47 service classes** — Full business logic with governance rules, board agent governance, resource propagation + lineage, bug-regression workflow, archive/restore, traceability and board-level resource readiness. Source: classes ending in `Service` under `core/services`.
...
- **334 MCP tools** — Complete Model Context Protocol command catalog for AI agent integration, counted from the transport-neutral Core catalog after importing the server, including:
  ...
  - Community runtime exposure: 334 core MCP tools, 0 community-only MCP tools
...
**→ [Relational ownership matrix](docs/architecture/af35_relational_ownership_matrix.md)** —
the executable AF-35 Core/edition ownership boundary and residual inventory.
...
| --- | --- | ---: | ---: |
| Core import rows | Core | 6666 | classified |
| Community-to-Core import rows | Community | 1019 | classified |
...
For plain-language definitions of recurring domain and architecture terms used in this repository, see the [Glossary](docs/GLOSSARY.md).
