...
class TestSemanticGraphStore:
    ...
    def test_get_schema_info(self):
        ...
        assert {"name": "violates", "from": "Bug", "to": "Constraint"} in violates
        assert len(violates) == 2
        assert next(rel["pairs"] for rel in violates if rel.get("multi")) == [
            {"from": "Bug", "to": "Requirement"},
            {"from": "Bug", "to": "Criterion"},
        ]
