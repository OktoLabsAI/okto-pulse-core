...
Covered: AST delegation + no raw session for all 15 board tools; the OUT-of-scope
boundary; the get_board / list_board_members aggregations + "Board not found";
...
def test_boundary_guideline_designsystem_defaults_stay_out():
    """The guideline/design-system DEFAULT/CRUD tools are OUT of the board family —
    they must NOT have been migrated (still open get_db_for_mcp)."""
    src = Path(mcp_server.__file__).read_text(encoding="utf-8")
    found = set()
    for n in ast.walk(ast.parse(src)):
        if isinstance(n, ast.AsyncFunctionDef):
            ...
            if short in _OUT_OF_SCOPE:
                found.add(short)
                block = ast.get_source_segment(src, n) or ""
                assert "async with get_db_for_mcp" in block, (
                    f"{short} is OUT of MCP-FU6 but appears migrated"
                )
    assert found == set(_OUT_OF_SCOPE), f"missing OUT tools: {set(_OUT_OF_SCOPE) - found}"
