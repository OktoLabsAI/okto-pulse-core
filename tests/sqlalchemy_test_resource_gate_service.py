"""Test-only SQLAlchemy adapter for the Core Resource Gate policy.
...
class TestSqlAlchemyResourceGateAdapter:
    """Persist and project Resource Gate evidence for Core tests."""
    ...
    def _resource_identity_values(item: Any) -> set[str]:
        ...
        if isinstance(item, dict):
            ...
            values.update(TestSqlAlchemyResourceGateAdapter._source_ref_values(item.get("source_ref")))
            values.update(TestSqlAlchemyResourceGateAdapter._source_ref_values(item.get("origin_ref")))
            values.update(TestSqlAlchemyResourceGateAdapter._source_ref_values(item.get("source")))
        elif item:
            values.add(str(item))
        return values
...
__all__ = ["TestSqlAlchemyResourceGateAdapter"]
